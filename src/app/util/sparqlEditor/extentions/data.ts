...
WHERE {
  ...
  "SIMILARITY": {
    type: "keyword",
    label: "SIMILARITY JOIN ON",
    description: "Performs a similarity join on the specified variables",
    example: `SELECT ?x ?y ?d WHERE {
  {?x v:valor ?xv }
  SIMILARITY JOIN ON (?xv) (?yv) TOP 3 DISTANCE sim:manhattanvec AS ?d
  {?y v:valor ?yv}
}`,
    link: "https://sferrada.com/publication/2024-sw-ferrada-simjoin/2024-SW-ferrada-simjoin.pdf",
    boost: 10
  },
  "CLUSTER": {
    type: "keyword",
    label: "CLUSTER BY",
    description: "Clusters the results based on the specified variables using the specified clustering algorithm",
    example: `SELECT ?star ?c WHERE {
  { SELECT ?s ?m ?r ?t (COUNT(DISTINCT ?p) AS ?n)
    WHERE { ?star rdf:type/rdfs:subClassOf* :Star ; :mass ?m ; :radius ?r ; :temperature ?t ; :child ?p . } }
  CLUSTER BY (?m ?r ?t ?n) WITH sim:kmeans(4) AS ?c
}`,
    link: "https://sferrada.com/publication/2024-sw-ferrada-simjoin/2024-SW-ferrada-simjoin.pdf",
    boost: 10
  }
